...
/**
 * Class responsible for transforming NUnit to JUnit files and then run them all
 * through the JUnit result archiver.
 *
 * @author Erik Ramfelt
 */
public class NUnitArchiver implements FilePath.FileCallable<Boolean>, Serializable {
    ...
    private String[] findNUnitReports(File parentPath) {
        ...
        if (nunitFiles.length == 0) {
            ...
            if (this.failIfNoResults) {
                ...
                fatalError("No NUnit test report files were found. Configuration error?");
            } else {
                getLogger().println("No NUnit test report files were found.");
            }
        }
        return nunitFiles;
    }
    ...
    private void fatalError(String message) {
        if (this.listener != null) {
            this.listener.fatalError(message);
        } else {
            this.taskListener.fatalError(message);
        }
    }
