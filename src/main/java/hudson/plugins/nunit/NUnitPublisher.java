...
public class NUnitPublisher extends Recorder implements Serializable, SimpleBuildStep {
    ...
    /**
     * <p>
     * Flag that when set, <strong>marks the build as failed if there are no
        ...
        try {
            ...
            if (filePath.act(transformer)) {
                ...
                if (skipJUnitArchiver) {
                    ...
                    // Run the JUnit test archiver
                    recordTestResult(NUnitArchiver.JUNIT_REPORTS_PATH + "/TEST-*.xml", run, taskListener, filePath);
                }
    ...
    private boolean recordTestResult(String junitFilePattern, Run<?, ?> run, TaskListener listener, FilePath filePath)
        ...
        TestResult result = getTestResult(junitFilePattern, filePath, existingTestResults, buildTime);
